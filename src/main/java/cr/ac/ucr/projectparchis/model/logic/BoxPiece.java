...
public class BoxPiece {
    ...
    private boolean doubleOccupied = false;
    ...
    private void calcMid() {
        midCord[0] = (boxPoint1[0] + boxPoint2[0]) / 2.0;
        midCord[1] = (boxPoint1[1] + boxPoint2[1]) / 2.0;
    }

    private void calcLeftPoint() {
        leftCord[0] = (boxPoint1[0] + midCord[0]) / 2.0;
        leftCord[1] = midCord[1];
    }

    private void calcRightPoint() {
        rightCord[0] = (midCord[0] + boxPoint2[0]) / 2.0;
        rightCord[1] = midCord[1];
    }

    private void calcHomeCords() {
        double distY = Math.abs(midCord[1] - boxPoint1[1]) / 2.0; // mitad del semialto
        // superior izquierda
        homeCords[0] = leftCord[0];
        homeCords[1] = midCord[1] - distY;
        // superior derecha
        homeCords[2] = rightCord[0];
        homeCords[3] = midCord[1] - distY;
        // inferior izquierda
        homeCords[4] = leftCord[0];
        homeCords[5] = midCord[1] + distY;
        // inferior derecha
        homeCords[6] = rightCord[0];
        homeCords[7] = midCord[1] + distY;
    }
    ...
    public boolean isDoubleOccupied() {
        return doubleOccupied;
    }
    ...
    public void setDoubleOccupied(boolean value) {
        this.doubleOccupied = value;
    }
